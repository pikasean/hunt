(function() {
  ...
  function checkAnswer(puzzleId, submission) {
    ...
    var instructionHashCode = instructionHashCodes[puzzleId];
  ...
  $(function() {
    const puzzleId = $('#checkAnswerModal').attr('data-puzzle-id');

    $('#checkAnswerForm').submit(function(evt) {
      evt.preventDefault();

      var guess = $('#checkAnswerForm input').val();

      resetModal();
      $('#checkAnswerForm input').focus();

      var result = checkAnswer(puzzleId, guess);
      if (result.correct) {
        $('#checkAnswerResult')
          .text('Correct!')
          .addClass('correct');
